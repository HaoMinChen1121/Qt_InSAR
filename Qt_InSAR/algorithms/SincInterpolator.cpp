                              ...
                              QVector<std::complex<float>>& dst, int dstW)
{
    ...
    sar::ComplexSoA srcSoA;
    srcSoA.fromAos(src.constData(), totalSrc);
    sar::ComplexSoAView srcView = srcSoA.view(0, totalSrc);

    sar::ComplexSoA dstSoA;
    sar::sincInterp1D_Horizontal_SoA(srcView, srcW, srcH, sx, weightLUT,
                                      sincWin, dstSoA, dstW);

    dst.resize(srcH * dstW);
    dstSoA.toAos(dst.data(), srcH * dstW);
}
...
{
    int totalSrc = srcH * width;
    sar::ComplexSoA srcSoA;
    srcSoA.fromAos(src.constData(), totalSrc);
    sar::ComplexSoAView srcView = srcSoA.view(0, totalSrc);

    sar::sincInterp1D_Vertical_SoA(srcView, srcH, width, sy, weightLUT,
                                    sincWin, dst);
}
